...
public class MainView {
	...
	GoldkeyController gCon = new GoldkeyController();	//★비아추가★
	...
	void move(int player, int num) {
	 ...
	 // 수현 - 6. 이동한 땅의 주인 존재 여부 확인 메소드 [R]
	   void isExistLandlord(int whoIsTurn, int n_no) {
	      ...
	      int p_no=nCon.isExistLandlord(whoIsTurn,n_no);
	      
	      if(p_no==0) {//빈 땅일때
	         System.out.println("안내) 땅을 구입할 수 있습니다.");
	         System.out.print("안내) 땅을 구입하시겠습니까? [Y:1][N:2]"); int ch=sc.nextInt();
	         
	         if(ch==1) buyLand(whoIsTurn,n_no); // 사겠다고 하면 땅구매 메소드 이동
	         else return; // no 하면 턴종료
	      }
	         ...
	         System.out.println("안내) 안타깝게도 남의 땅입니다. 통행료를 지불해주세요.");
	...
	void buyLand(int player, int n_no) {
		...
		int n_price= nCon.getLandPrice(n_no);
		String n_name= nCon.getLandName(n_no);
		int p_money=pCon.getPlayerMoney(player);
		System.out.println(n_name +"의 가격은 "+n_price+"입니다.");
		if(n_price <= p_money){
			// 내 자산이 더 많으면 구매 실행
			boolean result = nCon.buyLand(player, n_no, n_price);
			// 수현 n_price 매개변수 추가!!! 구매하면 자산에서 빼줘야해서 땅가격 매개변수 추가!!
			if(result) {System.out.println("안내) 땅 구매 완료했습니다.");}
		}else {System.out.println("안내) 자산부족으로 실패했습니다.(아마...?)");}
